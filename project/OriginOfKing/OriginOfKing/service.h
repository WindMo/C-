...
public:
	...
	// 根据“代”查询
	virtual vector<Person>  queryByGeneration(int &number)= 0;
	// 查询孩子
	virtual vector<Person>  queryByChildren(string &personName)= 0;
	// 查询相同父亲的人
	virtual vector<Person>  queryByFatherName(string &fatherName)= 0;
	// 查询其兄弟
	virtual vector<Person>  queryBrother(string &personName)= 0;
	/*				添加			*/
	// 添加一个人并返回当前总数
...
public:
	...
	// 显示树
	virtual void show(vector<Person>  &personList) = 0;
};
